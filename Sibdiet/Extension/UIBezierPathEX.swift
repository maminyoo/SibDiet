...
extension UIBezierPath {
    ...
    static func createCircleAtPoint(origin: CGPoint, radius: CGFloat) -> UIBezierPath {
        let boundingRect = CGRect(origin.x - radius,
                                  origin.y - radius,
                                  radius * 2,
                                  radius * 2)
    ...
    func addCurve(_ to: CGPoint, _ point1: CGPoint, _ point2: CGPoint){
        addCurve(to: to, controlPoint1: point1, controlPoint2: point2)
    }
    
    func addLine(_ x: CGFloat, _ y: CGFloat){
