...
class AboutView: UIView, AboutViewDelegate{
    ...
    func setDr(){
        drView.frame(10, -102+statusHeight, 120, 300)
    ...
    func setBottomView(){
        let height:CGFloat = haveSafeArea ? 45 : 30
    ...
    func setDiets(){
        ...
        dietsView.onTap(self, #selector(tapDiets), 1)
        ...
        setDietsCloseGesture()
        addSubview(dietsView)
    }
    
    //MARK: CLOSE
    var closeGesture = UIView()
    func setDietsCloseGesture(){
        let x: CGFloat = 0,
        y: CGFloat = topBarView.y + topBarView.height/2 + 5,
        w: CGFloat = screenWidth,
        h: CGFloat = backDietDescription.y - y - dietDescription.height
        closeGesture.frame = CGRect(x: x, y: y, width: w, height: h)
        closeGesture.isHidden = true
        closeGesture.onTap(self, #selector(closer(tap:)))
        addSubview(closeGesture)
    }
    
    @objc func closer(tap: UIGestureRecognizer){
        if open && touchable{
            closeDiets()
        }
    }
    ...
    func openDiets(){
        if !open && touchable{
            touchable = false
            closeGesture.isHidden = false
            backButton.opacity = 0.2
            
            let backDietDescriptionMask01Y = CATransform3DTranslate(backDietDescriptionMaskOrigin, 0, -backDietDescriptionMask.height, 0)
            backDietDescriptionMask.animate(transform3D: backDietDescriptionMask01Y, 0.6, easeInOut05)
            
            let dietDescriptionMask01Y = CATransform3DTranslate(dietDescriptionMaskOrigin, 0, -dietDescription.height, 0)
            dietDescriptionMask.layer.animate(transform3D: dietDescriptionMask01Y, 0.6, easeInOut05)
            
            let diets01H = dietsOriginR.height + dietDescription.height
            let diets01T = CATransform3DTranslate(dietsOriginT, 0, -(dietDescription.height/2), 0)
            dietsView.animate(height: diets01H, 0.6, curveEaseInOut05)
            dietsView.animate(transform3D: diets01T, 0.6, curveEaseInOut05)
            dietsView.animate(backgroundColor: gray13, 0.8, curveEaseIn05)
            
            backButton.animate(opacity: 1, 0.7, curveEaseInOut, 0.9)
            
            let _: Timer = Timer.schedule(1.4) { _ in
                self.isOpen()
            }
        }else if open && touchable {
            closeDiets()
        }
    }
    ...
    @objc func closeView(){
        if touchable{
            ...
            let _: Timer = Timer.schedule(1.7) { _ in
                self.setFirstView()
            }
        }
    }
